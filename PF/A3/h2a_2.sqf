...
private _blanket=selectRandom["Land_Sleeping_bag_F","Land_Sleeping_bag_blue_F","Land_Sleeping_bag_brown_F"];
private _blanket=createVehicle[_blanket,[0,0,0],[],0,"can_collide"];_blanket enableSimulationGlobal false;
_paper=selectRandom["Leaflet_05_New_F","Leaflet_05_Old_F"];
private _paper=createSimpleObject[_paper,[0,0,0]];

_chairW1 attachTo[_house,[-3.2,-1.78,0.27]];_chairW1 setDir 250;
_shelfW1 attachTo[_house,[-.8,-2,.76]];_shelfW1 setDir 90;
_shelfW2 attachTo[_house,[-2.8,-2,.76]];_shelfW2 setDir 270;
_shelfW3 attachTo[_house,[1.75,-.5,.76]];
_shelfW4 attachTo[_house,[1.75,1,.76]];
_cashier attachTo[_house,[-2,0,0.27]];_cashier setDir 270; 
_plant1 attachTo[_house,[-0.5,-2,2.023]];_plant1 setDir(random 359);
_plant2 attachTo[_house,[-0.9,-2,2.023]];_plant2 setDir(random 359);
_plant3 attachTo[_house,[-2.7,-2,2.023]];_plant3 setDir(random 359);
_plant4 attachTo[_house,[-3.1,-2,2.027]];_plant4 setDir(random 359);
_plant5 attachTo[_house,[1.75,1,2.023]];_plant5 setDir(random 359);
_pot1 attachTo[_house,[-.8,-2,1.004]];_pot1 setDir(random 359);
_pot2 attachTo[_house,[-2.6,-2,0.69]];_pot2 setDir(random 359);
_pot3 attachTo[_house,[-3,-2,0.69]];_pot3 setDir(random 359);
_pot4 attachTo[_house,[1.75,-0.7,1]];_pot4 setDir(random 359);
_pot5 attachTo[_house,[1.75,-0.3,1]];_pot5 setDir(random 359);
_trash attachTo[_house,[3.15,4.6,0.78]]; 
_chairW1 attachTo[_house,[-3.2,0.5,0.27]];_chairW1 setDir 300;
_sacks1 attachTo[_house,[-3,6.25,.76]];
_sacks2 attachTo[_house,[-3.2,3.5,.76]];_sacks2 setDir 90;
_boxes attachTo[_house,[-3.42,6.53,4.9]];_boxes setDir 182;
_luggage attachTo[_house,[-2.28,3.98,4.66]];_luggage setDir 8;
_oldDesk attachTo[_house,[-3.14,4.45,4.57]];
_chairW2 attachTo[_house,[-3.4,4.25,4.15]];_chairW2 setDir (random 10)+180;
_papers attachTo[_house,[-3.67,4.5,5.05]];_papers setDir 90;
_notepad attachTo[_house,[-3.4,4.25,4.989]];_notepad setDir 260;
_pencil attachTo[_house,[-3.4,4.25,4.989]];_pencil setDir(random 50);
_paper attachTo[_house,[-3.1,4.35,4.977]];_paper setDir 105;
_bucket attachTo[_house,[-2.8,3.9,4.32]];_bucket setDir 140;
_bed attachTo[_house,[-2.5,1,4.32]];_bed setDir 90;
_blanket attachTo[_house,[-2.5,1,4.517]];_blanket setDir 270;
_rack attachTo[_house,[1.7,4.27,5.145]];_rack setDir 90;
_fridge attachTo[_house,[1.6,2.63,4.7]];_fridge setDir 90;
_pChair attachTo[_house,[-3.2,-3.15,4.62]];_pChair setDir(random -80)+90;
...
if(dayTime>18 || dayTime<5)then{
	...
	if(count _guyHome>0)then{
	private _light=createVehicle["#lightpoint",[0,0,0],[],0,"can_collide"];
	_light attachTo[_house,[-2,3.5,7.1]];
>>>>>>> 07f0a89e0c6448f803f3e8d796a0852406ba09ff
