...
_table attachTo[_house,[8.75,2,1.62]];
_desk attachTo[_house,[4.3,-.7,1.62]];_desk setDir 180;
_shelf attachTo[_house,[5.56,-.83,2]];_shelf setDir 180;
_chair1 attachTo[_house,[8.3,1.5,1.25]];_chair1 setDir (random 10)+270;
_chair2 attachTo[_house,[8.3,2.5,1.25]];_chair2 setDir (random 10)+270;
_chair3 attachTo[_house,[4.5,-.5,1.22]];
_bottle attachTo[_house,[8.28,2.15,2.08]];_bottle setDir (random 359);
_bottle1 attachTo[_house,[8.3,2.35,2.168]];_bottle1 setDir (random 359);
_bottle2 attachTo[_house,[8.4,1.35,2.17]];_bottle2 setDir (random 359);
_radio attachTo[_house,[8.7,2,2.115]];_radio setDir 280;
_stool attachTo[_house,[3,3.5,1.438]];_stool setDir 40;
_matches attachTo[_house,[3,3.75,1.708]];_matches setDir(random 359);
_logs attachTo[_house,[3.2,4.85,1.45]];_logs setDir 90;
_stump attachTo[_house,[1.315,3.93,1.465]];
_bed1 attachTo[_house,[-5,0,1.28]];_bed1 setDir 90;
_bed2 attachTo[_house,[-5.9,0,1.28]];_bed2 setDir 270;
_blanket1 attachTo[_house,[-5,0,1.696]];_blanket1 setDir 180;
_blanket2 attachTo[_house,[-5.9,0,1.696]];_blanket2 setDir 180;
_pillow1 attachTo[_house,[-5,-.8,1.742]];
_pillow2 attachTo[_house,[-5.9,-.8,1.742]];
_poster attachTo[_house,[-6.8,5.2,3]];
_axe attachTo[_house,[1.09,2.539,1.69]];_axe setDir 90;_axe setVectorUp[.8,.8,.2];
_bucket attachTo[_house,[0.4,-.7,1.42]];_bucket setDir 280;
_towels attachTo[_house,[0.4,-.48,1.28]];_towels setDir(random 359);
_crates attachTo[_house,[-.65,4.4,2]];_crates setDir 180;
_rug attachTo[_house,[7,.5,1.251]];
_carton attachTo[_house,[-9.4,-.7,1.54]];_carton setDir 270;
_gen attachTo[_house,[-9.3,1,1.55]];_gen setDir 90;
_wrench attachTo[_house,[-8.8,1.3,1.23]];_wrench setDir 305;
_glove attachTo[_house,[-8.9,1.5,1.23]];_glove setDir 260;
_basket attachTo[_house,[-5.5,1.45,1.55]];_basket setDir(random 359);
...
if(dayTime>18 || dayTime<5)then{
	...
	_fire1 attachTo[_house,[1.6,4.7,1.55]];
	sleep .5;
	detach _fire1;
	};
};
