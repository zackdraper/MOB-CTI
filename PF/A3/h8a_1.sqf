...
_desk attachTo[_house,[0,0.07,0.704]];_desk setDir 180;
_rack1 attachTo[_house,[1.1,-0.04,1.06]];_rack1 setDir 180;
_chair attachTo[_house,[0.2,0.25,0.28]];
_laptop attachTo[_house,[0.2,0.2,1.24]];_Laptop setDir 180;
_cup attachTo[_house,[0,0.3,1.16]];
_rack2 attachTo[_house,[3,4.1,0.78]];_rack2 setDir 90;
_blanket attachTo[_house,[-2.3,3.9,.31]];_blanket setDir 270;

if(dayTime>18 || dayTime<5)then{
	sleep 5;
	private _guyHome=_house nearEntities["Man",4];
	if(count _guyHome>0)then{
	private _fire1=createVehicle["MetalBarrel_burning_F",[0,0,0],[],0,"can_collide"];
	_fire1 attachTo[_house,[-3.05,.1,.7]];_fire1 setDir(random 359);
	sleep .5;
detach _fire1;};
};
