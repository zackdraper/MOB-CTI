...
_table attachTo[_house,[-4.45,-2.2,.5]];_table setDir 90;
_chair1 attachTo[_house,[-4.5,-1.8,.07]];_chair1 setDir 0;
_chair2 attachTo[_house,[-3.8,-2.3,.07]];_chair2 setDir(random 25)+90;
_rack attachTo[_house,[-1.9,2.95,.88]];_rack setDir 0;
_sack attachTo[_house,[-1.35,-2.36,.5]];_sack setDir 45;
_sacks attachTo[_house,[0,-1.7,.54]];_sacks setDir 40;
_basket attachTo[_house,[0.6,-.95,.41]];_basket setDir(random 359);
_blanket attachTo[_house,[-1.5,-1,.1]];_blanket setDir 180;
_graf attachTo[_house,[-5.37,1,1.5]];_graf setDir 270;
_trash attachTo[_house,[2.5,0,0.1]];_trash setDir(random 359);
_trashPile attachTo[_house,[4,-1.2,0.2]];_trashPile setDir(random 359);
_box1 attachTo[_house,[5,1.5,0.3]];_box1 setDir(random 359);
_box2 attachTo[_house,[4,0.6,0.3]];_box2 setDir(random 359);
...
if(dayTime>18 || dayTime<5)then{
	...
	_light attachTo[_house,[5.15,-2.4,0.5]];
	sleep .5;
detach _light;};
};
