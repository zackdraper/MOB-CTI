...
while { dialog && alive player } do {
	...
	if ( GRLIB_squadconfirm == 1 ) then {
		...
		if ( GRLIB_squadaction == 1 ) then {
			...
			_nearfob = [ getpos _selectedmember , "Barracks"] call F_getNearestCTIBuild;
			_fobdistance = 9999;
			...
			if ( _selectedmember distance _nearfob  < 100 ) then {

				loadout = [_selectedmember,configfile >> "CfgVehicles" >> (typeof _selectedmember)] call BIS_fnc_loadInventory;
		...
		if (GRLIB_squadaction == 4) then {
			...
			if ( _selectedmember distance player < 100 ) then {

				_vehplayer = vehicle player;
				
				[_vehplayer, _selectedmember, ""] call BIS_fnc_moveIn;

				hint localize 'STR_BOARD_OK';
			} else {
				hint localize 'STR_BOARD_KO';
			};
