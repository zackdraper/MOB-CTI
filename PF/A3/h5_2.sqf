...
_blanket1 attachTo[_house,[-1.1,-1.2,3]];_blanket1 setDir 270;
_blanket2 attachTo[_house,[-1,.3,3.03]];_blanket2 setDir 250;
_blanket3 attachTo[_house,[-1.6,2.7,3.04]];_blanket3 setDir 0;
_table attachTo[_house,[-1.05,-1,0.5]];_table setDir 270;
_stool attachTo[_house,[-1.05,-0.1,0.4]];_stool setDir 90;
_log1 attachTo[_house,[1,3.2,1.52]];_log1 setDir(random 359);
_log2 attachTo[_house,[0.45,3.45,1.52]];_log2 setDir(random 359);
_log3 attachTo[_house,[0.7,3.3,2.05]];_log3 setDir(random 359);
_stump attachTo[_house,[-.3,3.5,.425]];_stump setDir 90;_stump setVectorUp[0,1,0];
_logs attachTo[_house,[3.7,3.5,0.45]];_logs setDir 90;
_axe attachTo[_house,[0.4,3,1.27]];_axe setDir 40;
_graf1 attachTo[_house,[-2.17,.1,4]];_graf1 setDir 270;
_graf2 attachTo[_house,[0,3.825,4]];
_chair1 attachTo[_house,[4.7,-1.2,3.5]];_chair1 setDir(random 40)+90;
_chair2 attachTo[_house,[4.7,-0.1,3.5]];_chair2 setDir(random 40)+50;
...
if(dayTime>18 || dayTime<5)then{
	...
	if(count _guyHome>0)then{
	private _fire1=createVehicle["Fireplace_burning_F",[0,0,0],[],0,"can_collide"];
	_fire1 attachTo[_house,[0.47,3.05,.35]];
>>>>>>> 07f0a89e0c6448f803f3e8d796a0852406ba09ff
	sleep .5;
	detach _fire1;
	};
