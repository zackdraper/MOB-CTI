...
GOM_fnc_addAircraftLoadoutToObject = {
	...
	_action = ["Edit Aircraft Loadout",{params ["_caller"];[_caller] spawn GOM_fnc_aircraftLoadout},[],0,true,true,"","(vehicle _this) isEqualTo (vehicle _target) AND {!(isEngineOn _target)} AND {speed _this < 3} AND {alive _this} AND {alive _target}"];
