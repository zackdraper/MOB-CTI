CTI_UI_MapCommanding_GetOrdersParameters = {
	...
	switch (true) do {
		case (_order in [CTI_ORDER_TAKETOWNS, CTI_ORDER_TAKEHOLDTOWNS]): {
			_towns = MOB_CTI_Locations_AllMajorCities apply {name (_x select 0)};
			...
			{
				((uiNamespace getVariable "cti_dialog_ui_mapcommandmenu") displayCtrl 220010) lbAdd format["TOWN: %1", _x];
...
CTI_UI_MapCommanding_TrackGroupOrder = {
	...
	while {!isNil {uiNamespace getVariable "cti_dialog_ui_mapcommandmenu"}} do {
		...
		_mpos = _track getVariable ["cti_order_pos",[0,0]];
